...
>[!TIP]
>
>若要儲存自訂漏斗，請在配置設定以及執行報表後儲存 URL。您可以共用URL或將其儲存在檔案中。

以下為此報表的範例:

![](assets/funnel_create.png)

為展示簡單漏斗，以下是使用三個漏斗步驟和兩個漏斗比較的組態設定。 我們假設示範應用程式可讓使用者新增項目（例如像片），然後加以分享。

在「自訂」視窗中，有些區段可指出使用者啟動應用程式、從應用程式的圖庫新增像片、從應用程式分享一或多張像片至社交媒體、簡訊、電子郵件等。 漏斗比較可讓您比較iOS應用程式和Android應用程式使用者之間新增和分享像片的層級。
...
第一個系列顯示100%的使用者已啟動應用程式。 第二個系列顯示，較高比例的Android使用者從圖庫新增像片。 第三個系列顯示，幾乎一半的iOS使用者分享了像片，但是沒有一個Android使用者分享了像片。 這可能表示應用程式有問題，需要進行調查。

要顯示其他資訊，請將滑鼠移到圖表中的任意條上。

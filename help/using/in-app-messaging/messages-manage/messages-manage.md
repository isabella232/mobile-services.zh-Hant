...
您可以複製、啟用／停用以及封存／取消封存訊息。 您也可以檢視報表，其中顯示已檢視訊息的使用者人數，以及已點選訊息的使用者人數。

若要管理訊息，請按 ***一下your_app_name*** > **[!UICONTROL Messaging]** > **[!UICONTROL Manage Messages]**。

![](assets/manage_messages.png)

以下是您可針對訊息完成的其他工作相關資訊：

* **搜尋及篩選訊息**

   使用清單頂端的搜尋方塊來搜尋及篩選訊息。您在搜尋方塊中輸入的任何內容都會顯示清單中的相符訊息。 搜尋適用於所有表格欄。 例如，您可以鍵入 `Draft` 以僅顯示具有該狀態的消息。 您也可以輸入使用者的名稱以僅顯示該使用者所更新之訊息。
...
* **複製訊息**
   ...
   1. 鍵入消息的名稱。
      ...
      若要複製相同應用程式的訊息，請為訊息輸入新名稱。 如果您保留相同的名稱以覆寫原始訊息。 如果您是為不同應用程式複製訊息，則可維持相同名稱，不會覆寫原本的訊息。

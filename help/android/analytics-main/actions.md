---
description: 動作是發生在您Android應用程式中，您要測量的事件。
seo-description: 動作是發生在您Android應用程式中，您要測量的事件。
seo-title: 追蹤應用程式動作
...
動作是發生在您Android應用程式中，您要測量的事件。

每個動作都有一或多個對應度量，這些度量會在每次事件發生時遞增。 例如，您可能會針對每個新訂閱、每次檢視文章或每次完成某個層級時，傳送 `trackAction` 呼叫。應用程式不會自動追蹤動作，因次您必須在要追蹤的事件發生時呼叫 `trackAction`，然後將動作對應至自訂事件。
